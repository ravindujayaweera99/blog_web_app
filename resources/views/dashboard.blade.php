        ...
        <div class="py-12 flex flex-col justify-center items-center gap-12">
            <div class="w-full mx-auto sm:px-6 lg:px-8">
                <div class="overflow-hidden flex flex-col md:flex-row justify-center items-center gap-6">
                    <div
                        class="bg-white shadow-lg rounded-lg border border-purple-200 flex flex-col p-6 w-full max-w-sm">
                            ...
                            <div
                                ...
                                <div class="flex flex-col md:flex-row justify-between items-center w-full">
                                    <div class="flex flex-col md:flex-row md:gap-6 items-center">
                                        <div class="font-bold text-md">{{ $userPost->title }}</div>
                                        ...
                                        @endif
                                        <div class="text-[11px] font-bold">Published
                                            {{ $userPost->created_at->diffForHumans() }}
                                        </div>
                                    </div>
                                </div>
                                <div class="flex gap-4">
                                    ...
                                    <form action="{{ route('post.destroy', $userPost->id) }}" method="post"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="delete-button border bg-red-500 border-red-500 rounded text-white px-2 py-1 hover:bg-red-600  transition duration-150 ease-in-out">Delete</button>
                                    </form>
                                </div>
                            </div>
                        @endforeach
